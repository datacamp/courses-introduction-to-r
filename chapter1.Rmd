...
- Select `3 + 4` in the editor, then press `CTRL` + `ENTER` to run the selection (`CMD` + `ENTER` on macOS).
- Add a line of code that calculates the sum of 6 and 12, and hit the 'Submit Answer' button or press `CTRL` + `SHIFT` + `ENTER` to run all your code and test the result (`CTRL` + `SHIFT` + `ENTER` on macOS).
