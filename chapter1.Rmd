...
- Most numbers that you will encounter in R have type **numeric**.  These include numbers with decimals, like 4.5, as well as whole numbers, like -4.
- If you only need to work with whole numbers, there is another type called **integer**.  These take up less memory than numeric variables, so they are occasionally useful where this is important.  Most of the time, R takes care of how numbers are handled, so you don’t need to worry about the difference between numeric and integer.
- The logical values `TRUE` and `FALSE` have type **logical**.  `TRUE` and `FALSE` must always be written in upper-case letters.  They are sometimes referred to as Boolean variables.
- Text values, sometimes known as "strings" have type **character**. They are written inside double quotes,`"`, or single quotes, `'`.
...
success_msg("Great work! Numbers are stored in `numeric` variables, `TRUE` and `FALSE` are stored in `logical` variables, and text is stored in `character` variables.")
```
...
Do you remember that when you added `5 + "six"`, you got an error due to a mismatch in data types? You can avoid such embarrassing situations by checking the data type of a variable beforehand. You can do this with the [`class()`](https://www.rdocumentation.org/packages/base/topics/class) function, as the code on the right shows.
