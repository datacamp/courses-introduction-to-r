...
*** =instructions
- The code to make a factor vector from `survey_vector` is coded for you, check it out. You should use `factor_survey_vector` in the next instruction.
- Change the factor levels of `factor_survey_vector` to `"Female"` and `"Male"`. Mind the order of the vector elements here.
...
test_function("factor","x",allow_extra = FALSE,
              not_called_msg = msg,
              incorrect_msg = msg)
...
After finishing this course, one of your favorite functions in R will be [`summary()`](http://www.rdocumentation.org/packages/base/functions/summary). This will give you a quick overview of `some_variable`: 
...
Since `"Male"` and `"Female"` are non-ordered (or nominal) factor levels, R returns a warning message, telling you that the greater than operator is not meaningful. As seen before, R attaches an equal value to the levels for such factors. 
...
`speed_vector` should be converted to an ordinal factor since its categories have a natural ordering. By default, the function [`factor()`](http://www.rdocumentation.org/packages/base/functions/factor) transforms `speed_vector` into an non-ordered factor. To create an ordered factor, you have to add two additional arguments: `ordered` and `levels`. 
