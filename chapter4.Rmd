...
title       : Factors
description : Very often, data falls into a limited number of categories. For example, humans identify as masculine or feminine (or non-binary). In R, categorical data is stored in factors. Given the importance of these factors in data analysis, you should start learning how to create, subset and compare them now!
...
A good example of a categorical variable is the variable 'Gender'. A human individual can either be "Masculine" or "Feminine", making abstraction of those who do not identify as either of these binary options. So here "Masculine" and "Feminine" are, in a simplified sense, the two values of the categorical variable "Gender", and every observation can be assigned to either the value "Masculine" of "Feminine".
...
To create factors in R, you make use of the function [`factor()`](http://www.rdocumentation.org/packages/base/functions/factor). First thing that you have to do is create a vector that contains all the observations that belong to a limited number of categories. For example, `gender_vector` contains the gender of 5 different individuals:

```
gender_vector <- c("Masculine","Feminine","Feminine","Masculine","Masculine")
```

It is clear that there are two categories, or in R-terms **'factor levels'**, at work here: "Masculine" and "Feminine".
...
Next, when you want to start your data analysis, your main concern is to keep a nice overview of all the variables and what they mean. At that point, you will often want to change the factor levels to `"Masculine"` and `"Feminine"` instead of `"M"` and `"F"` to make your life easier.

**Watch out:** the order with which you assign the levels is important. If you type `levels(factor_survey_vector)`, you'll see that it outputs `[1] "F" "M"`. If you don't specify the levels of the factor when creating the vector, `R` will automatically assign them alphabetically. To correctly map `"F"` to `"Feminine"` and `"M"` to `"Masculine"`, the levels should be set to `c("Feminine", "Masculine")`, in this order order.
...
- Change the factor levels of `factor_survey_vector` to `c("Feminine", "Masculine")`. Mind the order of the vector elements here.
...
# Specify the levels of factor_survey_vector
levels(factor_survey_vector) <- c("Feminine", "Masculine")
...
# test_student_typed('c("Masculine", "Feminine")')

test_object("factor_survey_vector", eq_condition = "equal",
            incorrect_msg = paste("Did you assign the correct factor levels to `factor_survey_vector`? Use `levels(factor_survey_vector) <- c(\"Feminine\", \"Masculine\")`. Remember that R is case sensitive!"))
...
Going back to our survey, you would like to know how many `"Masculine"` responses you have in your study, and how many `"Feminine"` responses. The [`summary()`](http://www.rdocumentation.org/packages/base/functions/summary) function gives you the answer to this question.
...
success_msg("Nice! Have a look at the output. The fact that you identified `\"Masculine\"` and `\"Feminine\"` as factor levels in `factor_survey_vector` enables R to show the number of elements for each category.")
```
...
In `factor_survey_vector` we have a factor with two levels: Masculine and Feminine. But how does R value these relatively to each other? In other words, what does R think is better, masculine or feminine?

*** =instructions
Read the code in the editor and click 'Submit Answer' to see whether those identifying as masculine are worth more than those who identify as feminine.
...
# Masculine
masculine <- factor_survey_vector[1]

# Feminine
feminine <- factor_survey_vector[2]

# Battle of the genders: Masculine 'larger' than feminine?
masculine > feminine
```
...
test_object("survey_vector", undefined_msg = msg, incorrect_msg = msg)
test_object("factor_survey_vector", eq_condition = "equal", undefined_msg = msg, incorrect_msg = msg)
test_object("masculine", undefined_msg = msg, incorrect_msg = msg)
test_object("feminine", undefined_msg = msg, incorrect_msg = msg)
test_output_contains("masculine > feminine", incorrect_msg = msg)
...
Since `"Masculine"` and `"Feminine"` are unordered (or nominal) factor levels, R returns a warning message, telling you that the greater than operator is not meaningful. As seen before, R attaches an equal value to the levels for such factors.
