...
description: >-
  ...
  rosso, grigio o bianco, e forse qualche altra opzione per le persone che si
  colorano i capelli. In R, i dati categorici vengono memorizzati in fattori. I
  fattori sono molto importanti nell'analisi dei dati, quindi inizia subito a
  imparare come crearli, suddividerli e confrontarli.
---
...
In questo capitolo ti immergi nel meraviglioso mondo dei **fattori**.
...
È importante che R sappia se ha a che fare con una variabile continua o categorica, poiché i modelli statistici che svilupperai in futuro trattano entrambi i tipi di variabili in modo diverso. (Vedrai più avanti perché questo è il caso).

Un buon esempio di variabile categorica è il sesso. In molte circostanze puoi limitare le categorie di sesso a "Maschio" o "Femmina". (A volte possono essere necessarie diverse categorie. Ad esempio, potrebbe essere necessario considerare le variazioni cromosomiche, gli animali ermafroditi o le diverse norme culturali, ma si avrà sempre un numero finito di categorie).

`@instructions`
Assegna alla variabile `theory` il valore `"factors"`.

`@hint`
Assegna semplicemente una variabile (`<-`); assicurati di scrivere correttamente in maiuscolo.
...
Per creare fattori in R, si utilizza la funzione `factor()`. La prima cosa da fare è creare un vettore che contenga tutte le osservazioni che appartengono a un numero limitato di categorie. Ad esempio, `sex_vector` contiene il sesso di 5 individui diversi:

```
sex_vector <- c("Male","Female","Female","Male","Male")
```

È chiaro che qui sono in gioco due categorie o, in termini di R, **"livelli di fattori"**: <span translate="no">"Male"</span> e <span translate="no">"Female"</span>.
...
- Stampa `factor_sex_vector` e asserisci che R stampa i livelli dei fattori al di sotto dei valori reali.

`@hint`
Basta utilizzare la funzione `factor()` su `sex_vector`. Dai un'occhiata al compito, la risposta è già lì da qualche parte...
...
Una variabile nominale è una variabile categorica senza un ordine implicito. Ciò significa che è impossibile dire che "uno vale più dell'altro". Ad esempio, pensiamo alla variabile categorica `animals_vector` con le categorie `"Elephant"`, `"Giraffe"`, `"Donkey"` e `"Horse"`. In questo caso, è impossibile dire che uno sta sopra o sotto l'altro. (Nota che alcuni di voi potrebbero non essere d'accordo ;-) ).
...
`@hint`
Invia la risposta e guarda la console. Nota come R indica l'ordine dei livelli dei fattori per le variabili categoriali ordinali.
...
Quando ricevi per la prima volta un set di dati, noterai spesso che contiene fattori con livelli specifici. Tuttavia, a volte vorrai cambiare i nomi di questi livelli per chiarezza o per altri motivi. R ti permette di farlo con la funzione `levels()`:

```
levels(factor_vector) <- c("name1", "name2",...)
```

Un buon esempio sono i dati grezzi che ti vengono forniti da un sondaggio. Una domanda comune a tutti i questionari è il sesso dell'intervistato. Qui, per semplicità, sono state registrate solo due categorie, `"M"` e `"F"`. (Di solito servono più categorie per i dati di un sondaggio; in ogni caso, si utilizza un fattore per memorizzare i dati categorici). 

```
survey_vector <- c("M", "F", "F", "M", "M")
```

Registrare il sesso con le abbreviazioni `"M"` e `"F"` può essere comodo se stai raccogliendo i dati con carta e penna, ma può creare confusione in fase di analisi dei dati. A quel punto, spesso vorrai cambiare i livelli dei fattori in `"Male"` e `"Female"` invece di `"M"` e `"F"` per maggiore chiarezza.

**Attenzione:** l'ordine di assegnazione dei livelli è importante. Se digiti `levels(factor_survey_vector)`, vedrai che viene visualizzato `[1] "F" "M"`. Se non specifichi i livelli del fattore quando crei il vettore, `R` li assegnerà automaticamente in ordine alfabetico. Per mappare correttamente `"F"` su `"Female"` e `"M"` su `"Male"`, i livelli devono essere impostati su `c("Female", "Male")`, in questo ordine.

`@instructions`
- Guarda il codice che costruisce un vettore di fattori da `survey_vector`. Dovresti utilizzare `factor_survey_vector` nella prossima istruzione.
- Cambia i livelli dei fattori di `factor_survey_vector` in `c("Female", "Male")`. Fai attenzione all'ordine degli elementi vettoriali.
...
Tornando al nostro sondaggio, vorresti sapere quante risposte `"Male"` hai nel tuo studio e quante risposte `"Female"`. La funzione `summary` fornisce la risposta a questa domanda.

`@instructions`
Chiedi a `summary()` di `survey_vector` e `factor_survey_vector`. Interpreta i risultati di entrambi i vettori. Sono entrambi ugualmente utili in questo caso?

`@hint`
Chiama la funzione `summary()` sia su `survey_vector` che su `factor_survey_vector`, è semplicissimo!
...
Potresti chiederti cosa succede quando cerchi di confrontare gli elementi di un fattore.  In `factor_survey_vector` c'è un fattore con due livelli: `"Male"` e `"Female"`. Ma come valuta R questi elementi l'uno rispetto all'altro?

`@instructions`
Leggi il codice nell'editor e invia la risposta per verificare se `male` è maggiore di (`>`) `female`.

`@hint`
Invia la risposta e guarda l'output che viene stampato nella console.
...
Poiché `"Male"` e `"Female"` sono livelli di fattori non ordinati (o nominali), R restituisce un messaggio di avvertimento che indica che l'operatore maggiore di non è significativo. Come visto in precedenza, R attribuisce un valore uguale ai livelli di tali fattori.

Ma non è sempre così! A volte si ha a che fare anche con fattori che hanno un ordine naturale tra le categorie. Se questo è il caso, dobbiamo assicurarci di trasmettere queste informazioni a R...

Supponiamo che tu sia a capo di un team di ricerca composto da cinque analisti di dati e che tu voglia valutare le loro prestazioni. A tal fine, si tiene traccia della loro velocità, si valuta ogni analista come `"slow"`, `"medium"` o `"fast"`, e si salvano i risultati in `speed_vector`.

`@instructions`
Come primo passo, assegniamo a `speed_vector` un vettore con 5 voci, una per ogni analista. Ogni voce deve essere o `"slow"`, o `"medium"`, o `"fast"`. Usa l'elenco qui sotto:

- L'analista 1 è medio,
- L'analista 2 è lento,
- L'analista 3 è lento,
- L'analista 4 è medio e
- L'analista 5 è veloce.
...
`speed_vector` dovrebbe essere convertito in un fattore ordinale poiché le sue categorie hanno un ordine naturale. Per impostazione predefinita, la funzione `factor()` trasforma `speed_vector` in un fattore non ordinato. Per creare un fattore ordinato, devi aggiungere due argomenti: `ordered` e `levels`.
...
Impostando l'argomento `ordered` a `TRUE` nella funzione `factor()`, indichi che il fattore è ordinato. Con l'argomento `levels` fornisci i valori del fattore nell'ordine corretto.

`@instructions`
Da `speed_vector`, crea un vettore di fattori ordinati: `factor_speed_vector`. Imposta `ordered` su `TRUE`, e imposta `levels` su `c("slow", "medium", "fast")`.

`@hint`
Usa la funzione `factor()` per creare `factor_speed_vector` sulla base di `speed_character_vector`. L'argomento `ordered` deve essere impostato su `TRUE` poiché esiste un ordine naturale. Inoltre, imposta `levels = c("slow", "medium", "fast")`.
...
Dopo una brutta giornata di lavoro, l'"analista di dati numero due" entra nel tuo ufficio e inizia a lamentarsi che l'"analista di dati numero cinque" sta rallentando l'intero progetto. Dato che sai che l'"analista di dati numero due" ha la reputazione di essere un sapientone, decidi innanzitutto di verificare se la sua affermazione è vera.

Il fatto che `factor_speed_vector` sia ora ordinato ci permette di confrontare elementi diversi (gli analisti dei dati in questo caso). Puoi farlo semplicemente utilizzando i noti operatori.

`@instructions`
- Usa `[2]` per selezionare da `factor_speed_vector` il valore del fattore per il secondo analista di dati. Conservalo come `da2`.
- Usa `[5]` per selezionare il valore del fattore `factor_speed_vector` per il quinto analista di dati. Conservalo come `da5`.
- Controlla se `da2` è maggiore di `da5`; stampa semplicemente il risultato. Ricorda che puoi utilizzare l'operatore `>` per verificare se un elemento è più grande dell'altro.

`@hint`
- Per selezionare il valore del fattore per il terzo analista di dati, è necessario `factor_speed_vector[3]`.
- Per confrontare due valori, puoi utilizzare `>`. Ad esempio:
