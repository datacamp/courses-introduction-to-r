...
It is important that R knows whether it's dealing with a continuous or a categorical variable. This, since the statistical models you will develop in the future threat both in a different way. (You will see later why this is the case).
...
*** =sct
```{r eval=FALSE}
DM.result <- DM.closedTest("theory","c('R uses factors for categorical variables!')")
```
...
*** =sct
```{r eval=FALSE}
DM.result <- DM.closedTest( "factor.gender.vector", "factor(gender.vector)" )
```
...
DM.result <- DM.closedTest(names,values,check.existence=c(T,T,F))
```
...
*** =sct
```{r eval=FALSE}
DM.result <- DM.codeTest(c("summary(survey.vector)","summary(factor.survey.vector)"))
```
