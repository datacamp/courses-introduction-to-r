...
`@hint`
¡Sólo tienes que enviar la respuesta!
...
`@instructions`
Sólo tienes que enviar la respuesta para iniciar el primer ejercicio de las listas.
...
success_msg("¡Maravilloso! Dirígete al siguiente ejercicio.")
```
...
`@instructions`
- Cambia el código del ejercicio anterior (ver editor) añadiendo nombres a los componentes. Utiliza para `my_vector` el nombre `vec`, para `my_matrix` el nombre `mat` y para `my_df` el nombre `df`. 
...
ex() %>% check_object("my_list") %>% check_equal(eq_condition = "equal",incorrect_msg = "Parece que `my_list` no contiene los nombres correctos para los componentes. Asegúrate de usar los nombres `vec`, `mat` y `df`, respectivamente. Consulta la pista si tienes dificultades.")
...
`@hint`
`shining_list <- list(moviename = mov)` es sólo una parte de la solución; tu trabajo consiste en añadir también `act` y `rev` a la lista, con los nombres adecuados.
...
ex() %>% check_object("shining_list") %>% check_equal(incorrect_msg = "Parece que `shining_list` no contiene los elementos correctos: el primer elemento debe ser `mov`, el segundo elemento `act`, y el tercero `rev`.")

ex() %>% check_object("shining_list") %>% check_equal(eq_condition = "equal",incorrect_msg = "Parece que `shining_list` no contiene los nombres correctos para los componentes: nombra el primer elemento `moviename`, el segundo elemento `actors`, y el tercer elemento `reviews`.")
...
Una forma de seleccionar un componente es utilizando la posición numerada de ese componente. Por ejemplo, para "coger" el primer componente de `shining_list` escribe 
...
`@instructions`
- Selecciona en `shining_list` el vector que representa a los actores. Sólo tienes que imprimir este vector.
...
`@sct`
```{r}
msg <- "¡No elimines ni cambies la definición de `shining_list`, que está pre-cargada en el espacio de trabajo!"
ex() %>% check_object("shining_list", undefined_msg = msg) %>% check_equal(incorrect_msg = msg)

ex() %>% check_output_expr("shining_list$actors", missing_msg = "¿Has seleccionado y mostrado correctamente el vector que representa a los actores? Puedes usar `shining_list$actors`, por ejemplo.")

ex() %>% check_output_expr("shining_list$actors[2]", missing_msg = "Para seleccionar al segundo actor del vector que representa a los actores, debes encadenar tus selecciones: `shining_list$actors` representa a los actores, por lo que puedes añadir un `[2]` para seleccionar el segundo elemento.")

success_msg("¡Genial! Seleccionar elementos de listas es bastante fácil, ¿verdad? Continúa con el siguiente ejercicio.")
```
...
Has encontrado críticas de otra película más reciente de Jack Nicholson: ¡Los Departed!

| Puntuaciones | Comentarios               |
|--------|------------------------|
| 4.6    | Lo volvería a ver |
| 5      | ¡Asombroso!               |
| 4.8    | Me ha gustado             |
...
- Crea una lista, llamada `departed_list`, que contenga el marco de datos `movie_title`, `movie_actors`, reseñas como `reviews_df`, y la puntuación media de las reseñas como `avg_review`, e imprímela.
