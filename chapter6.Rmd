...
```
my_list <- list(name1 = your_component1, name2 = your_component2 ...)
``` 
...
```
my_list <- list(your_component1, your_component2 ...)
names(my_list) <- c("name1", "name2" ...)
```

*** =instructions
Change the code of the previous exercise (see editor) by adding names to the components. Use for `my_matrix` the name `mat`, for `my_vector` `vec` and for `my_df` `df`. Look at the console output to see how R prints a list.
...
success_msg("Great! You've successfully completed the chapter on lists!")
```
