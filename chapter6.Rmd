...
`@instructions`
- Add the vectors containing the new reviews to the existing `reviews` data frame and call it `extra_reviews`.
...
`@solution`
```{r}
# reviews_scores <- extra_reviews$scores
# avg_review <- mean(reviews_scores)
```
...
`@instructions`
- Create the `shining_list_full` object, that contains
...
# shining_list_full <- list(moviename, actors, extra_reviews, avg_review)
```
