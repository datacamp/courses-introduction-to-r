...
title: Elenchi
description: >-
  A differenza dei vettori, le liste possono contenere componenti di diverso
  tipo, proprio come le tue liste di cose da fare possono contenere diverse
  categorie di compiti. Questo capitolo ti insegnerà a creare, nominare e
  suddividere queste liste.
---
...
Congratulazioni! A questo punto del corso hai già acquisito una certa dimestichezza:

- **Vettori** (array monodimensionali): possono contenere valori numerici, caratteri o logici. Gli elementi di un vettore hanno tutti lo stesso tipo di dati.
- **Matrici** (array bidimensionali): possono contenere valori numerici, caratteri o logici. Gli elementi di una matrice hanno tutti lo stesso tipo di dati.
- **Cornici di dati** (oggetti bidimensionali): possono contenere valori numerici, caratteri o logici. All'interno di una colonna tutti gli elementi hanno lo stesso tipo di dati, ma colonne diverse possono avere tipi di dati diversi.

Piuttosto carino per un principiante di R, vero? ;-)

`@instructions`
Invia la risposta per iniziare a scoprire tutto sulle liste!
...
Un **elenco** in R è simile alla tua lista di cose da fare al lavoro o a scuola: le diverse voci dell'elenco differiscono per lunghezza, caratteristiche e tipo di attività da svolgere. 

Un elenco in R ti permette di riunire una serie di oggetti sotto un unico nome (cioè il nome dell'elenco) in modo ordinato. Questi oggetti possono essere matrici, vettori, cornici di dati, persino altre liste, ecc. Non è nemmeno necessario che questi oggetti siano collegati tra loro in qualche modo.

Si potrebbe dire che una lista è una sorta di super tipo di dati: puoi memorizzare praticamente qualsiasi informazione al suo interno!

`@instructions`
Basta inviare la risposta per iniziare il primo esercizio sulle liste.
...
Creiamo la nostra prima lista! Per costruire un elenco si utilizza la funzione `list()`: 

```
my_list <- list(comp1, comp2 ...)
```

Gli argomenti della funzione elenco sono i componenti dell'elenco. Ricorda che questi componenti possono essere matrici, vettori, altre liste, ...

`@instructions`
Costruisce una lista, denominata `my_list`, che contiene le variabili `my_vector`, `my_matrix` e `my_df` come componenti della lista.
...
Proprio come nella tua lista di cose da fare, vuoi evitare di non sapere o ricordare a cosa corrispondono i componenti della tua lista. Ecco perché dovresti dare loro dei nomi: 
...
In questo modo si crea un elenco con i componenti denominati `name1`, `name2` e così via. Se vuoi dare un nome alle tue liste dopo averle create, puoi utilizzare la funzione `names()` come hai fatto con i vettori. I seguenti comandi sono del tutto equivalenti all'assegnazione precedente:
...
- Stampa `my_list` in modo da poter ispezionare l'output.

`@hint`
Il primo metodo per assegnare i nomi ai componenti dell'elenco è il più semplice. Inizia così: 
```
my_list <- list(vec = my_vector)
```
Aggiungi gli altri due componenti in modo simile.
...
Essendo un grande appassionato di cinema (ricorda il tuo lavoro alla LucasFilms), decidi di iniziare ad archiviare informazioni sui film più belli con l'aiuto di liste. 

Inizia creando una lista per il film "Shining". Abbiamo già creato le variabili `mov`, `act` e `rev` nel tuo spazio di lavoro R. Non esitare a dare un'occhiata alla console.
...
- `reviews`: un frame di dati che contiene alcune recensioni (memorizzate in `rev`)

Non dimenticare di nominare i componenti dell'elenco di conseguenza (i nomi sono `moviename`, `actors` e `reviews`).

`@hint`
`shining_list <- list(moviename = mov)` è solo una parte della soluzione; il tuo compito è quello di aggiungere anche `act` e `rev` all'elenco, con i nomi appropriati.
...
## Selezione di elementi da un elenco
...
La tua lista sarà spesso composta da numerosi elementi e componenti. Pertanto, ottenere un singolo elemento, più elementi o un componente non è sempre semplice. 

Un modo per selezionare un componente è utilizzare la posizione numerata del componente stesso. Ad esempio, per "afferrare" il primo componente di `shining_list` digita 

```
shining_list[[1]]
```

Un modo rapido per verificarlo è digitarlo nella console. È importante ricordare che per selezionare gli elementi di un vettore si usano le parentesi quadre singole: `[ ]`. Non confonderli!

Puoi anche fare riferimento ai nomi dei componenti con `[[ ]]` o con il simbolo `$`. Entrambi selezioneranno il frame di dati che rappresenta le recensioni: 
...
Oltre a selezionare i componenti, spesso è necessario selezionare elementi specifici all'interno di questi componenti. Ad esempio, con `shining_list[[2]][1]` si seleziona dal secondo componente, `actors` (`shining_list[[2]]`), il primo elemento (`[1]`). Se digiti questa frase nella console, vedrai che la risposta è Jack Nicholson.

`@instructions`
- Seleziona da `shining_list` il vettore che rappresenta gli attori. Basta stampare questo vettore.
- Seleziona da `shining_list` il secondo elemento del vettore che rappresenta gli attori. Esegui una stampa come in precedenza.

`@hint`
- Per selezionare il vettore che rappresenta gli attori, puoi utilizzare `$actors`.
- Per selezionare il terzo elemento del vettore che rappresenta gli attori, usa `shining_list$actors[3]`. Cosa deve cambiare per selezionare il secondo elemento?
...
## Creare un nuovo elenco per un altro film
...
Hai trovato le recensioni di un altro film di Jack Nicholson, più recente: The Departed!
...
Sarebbe utile raccogliere tutte le informazioni sul film, come il titolo, gli attori e le recensioni in un'unica variabile. Poiché questi dati hanno forme diverse, è naturale combinarli in una variabile elenco.

`movie_title`, contenente il titolo del film, e `movie_actors`, contenente i nomi di alcuni attori del film, sono disponibili nell'area di lavoro.
...
- Unisci i vettori `scores` e `comments` in un frame di dati chiamato `reviews_df`. 
- Crea un elenco, chiamato `departed_list`, che contenga i frame di dati `movie_title`, `movie_actors`, le recensioni come `reviews_df`, e il punteggio medio delle recensioni come `avg_review`, e stampalo.

`@hint`
- Utilizzerai la funzione `c()` per creare i vettori. 
