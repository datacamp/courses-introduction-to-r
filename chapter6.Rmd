...
Une liste dans R vous permet de rassembler une diversité d'objets sous un seul nom (c'est-à-dire le nom de la liste) de manière ordonnée. Ces objets peuvent être des matrices, des vecteurs, des cadres de données, voire d'autres listes, etc. Il n'est même pas nécessaire que ces objets soient liés les uns aux autres de quelque manière que ce soit.
...
`@sct`
```{r}
success_msg("Cool. Passons à la pratique !")
```
...
ex() %>% check_object("my_list") %>% check_equal(eq_condition = "equal",incorrect_msg = "Il semble que `my_list` ne contient pas les noms corrects pour les composants. Assurez-vous d'utiliser les noms `vec`, `mat` et `df`, respectivement. Consultez l'indice si vous avez des difficultés.")
...
Outre la sélection de composants, vous devez souvent sélectionner des éléments spécifiques à partir de ces composants. Par exemple, avec `shining_list[[2]][1]`, vous sélectionnez à partir du deuxième composant, `actors` (`shining_list[[2]]`), le premier élément (`[1]`). Lorsque vous tapez ceci dans la console, vous verrez que la réponse est Jack Nicholson.
...
success_msg("Super ! Sélectionner des éléments dans des listes est plutôt facile, n'est-ce pas ? Continuez avec l'exercice suivant.")
```
...
| Scores | Commentaires               |
|--------|------------------------|
| 4,6    | I would watch it again (Je le regarderais à nouveau) |
| 5      | Amazing! (Incroyable !)               |
| 4,8    | I liked it (J'ai aimé)             |
| 5      | One of the best movies (Un des meilleurs films) |
| 4,2    | Fascinating plot (Une intrigue fascinante)       |
...
- Créez une liste, appelée `departed_list`, qui contient la trame de données `movie_title`, `movie_actors`, les critiques en tant que `reviews_df`, et la note moyenne des critiques en tant que `avg_review`, et affichez-la.
...
ex() %>% check_object("scores") %>% check_equal(eq_condition = "equal", incorrect_msg = "Avez-vous correctement créé le vecteur `scores` ?")
ex() %>% check_object("comments") %>% check_equal(eq_condition = "equal", incorrect_msg = "Avez-vous correctement créé le vecteur `comments` ?")
ex() %>% check_correct(
  check_object(.,"avg_review") %>% check_equal(eq_condition = "equal", incorrect_msg = "Avez-vous correctement créé l'objet `avg_review` ?"),
  check_function(.,"mean") %>% check_arg("x") %>% check_equal()
)
ex() %>% check_correct(
  check_object(.,"reviews_df") %>% check_equal(eq_condition = "equal", incorrect_msg = "Avez-vous correctement créé `reviews_df` ?"),
...
ex() %>% check_correct(
  check_object(.,"departed_list") %>% check_equal(eq_condition = "equal", incorrect_msg = "Avez-vous correctement créé `departed_list` ?"),
  check_function(.,"list") %>% {
    check_arg(.,"..1") %>% check_equal()
    check_arg(.,"..2") %>% check_equal()
...
ex() %>% check_output_expr("departed_list", missing_msg = "Avez-vous affiché le `departed_list` ?")
