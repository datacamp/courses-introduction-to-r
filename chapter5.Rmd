...
Une trame de données présente les variables d'un ensemble de données sous forme de colonnes et les observations sous forme de lignes. Ce concept sera familier à ceux qui utilisent différents logiciels statistiques tels que SAS ou SPSS.
...
Qu'avez-vous appris exactement dans les exercices précédents ? Vous avez sélectionné un sous-ensemble d'une trame de données (`planets_df`) en fonction de l'existence ou non d'une certaine condition (anneaux ou pas anneaux), et vous avez réussi à extraire toutes les données pertinentes. Plutôt impressionnant ! À l'heure actuelle, la NASA est probablement déjà en train d'examiner votre CV. 
