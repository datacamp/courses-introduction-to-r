...
- The columns of a data frames are sometimes called **fields**.
...
In this chapter you will work with the built-in example data frame [`mtcars`](http://www.rdocumentation.org/packages/datasets/functions/mtcars).

* Type `mtcars` to print the contents of that data frame.
...
*** =sct
```{r}
test_output_contains("mtcars", incorrect_msg = "You did not print `mtcars`. Make sure that you output `mtcars`.")
