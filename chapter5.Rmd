...
No capítulo sobre matrizes, você deve se lembrar que todos os elementos que colocamos em uma matriz devem ser do mesmo tipo. Na ocasião, seu conjunto de dados sobre Star Wars continha apenas elementos numéricos. 
...
`@sct`
```{r}
ex() %>% check_output_expr("mtcars", missing_msg = "Não altere nada no código. Certifique-se de que você está exibindo `mtcars`.")
...
Tanto ``tail()``head()` quanto  imprimem uma linha superior chamada de cabeçalho <i>(header),</i> que contém o nome das diferentes variáveis do conjunto de dados.

`@instructions`
Chame ``mtcars``head()` no conjunto de dados  para dar uma olhada no cabeçalho e nas primeiras observações.
...
ex() %>% check_output_expr("head(mtcars)", missing_msg = "Simplesmente imprima o resultado da chamada `head()`, não é necessário atribuí-lo a uma nova variável.")

success_msg("Maravilhoso! Então, o que temos neste conjunto de dados? Por exemplo, `hp` representa a potência do carro; o Datsun tem a menor potência dos 6 carros que são exibidos. Para uma visão completa do significado das variáveis, digite `?mtcars` no console e leia a página de ajuda. Continue para o próximo exercício!");
```
...
`@instructions`
Use a função ``data.frame()` para construir um quadro de dados. Passe os vetores `name`, `type`, `diameter`, `rotation` e `rings` como argumentos em `data.frame()`, nessa ordem. Chame o quadro de dados resultante de `planets_df`.
...
`@sct`
```{r}
msg = "Não altere nada na definição dos vetores. Apenas adicione uma chamada `data.frame()` para criar `planets_df`."
...
success_msg("Ótimo trabalho! O próximo passo lógico, como você já sabe, é inspecionar o data frame que você acabou de criar. Vá para o próximo exercício.");
...
`@instructions`
Use ``planets_df``str()` para investigar a estrutura da nova variável .
...
`@sct`
```{r}
msg = "Não remova ou sobrescreva o data frame `planets_df` que já está disponível no workspace!"
ex() %>% check_object("planets_df", undefined_msg = msg) %>% check_equal(incorrect_msg = msg)

ex() %>% check_output_expr("str(planets_df)", missing_msg = "Você exibiu corretamente a estrutura de `planets_df`? Use `str()` para fazer isso!")

success_msg("Incrível! Agora que você tem uma compreensão clara do conjunto de dados `planets_df`, é hora de ver como você pode selecionar elementos dele. Aprenda tudo sobre isso nos próximos exercícios!")
```
...
`@hint`
Para selecionar o diâmetro de Vênus (a segunda linha), você precisaria usar: `planets_df[2,3]`. Então, o que você precisa para Mercúrio?
...
ex() %>% check_output_expr('planets_df[rings_vector, ]', missing_msg = "Você adaptou corretamente o código para selecionar _todas_ as colunas dos planetas que têm anéis? Você pode usar `planets_df[rings_vector, ]`. Certifique-se de incluir a vírgula aqui, é crucial!")

success_msg("Maravilhoso! Esta é uma solução bastante tediosa. O próximo exercício ensinará como fazer isso de uma maneira mais concisa.")
```
...
O que você aprendeu exatamente nos exercícios anteriores? Você selecionou um subconjunto de um quadro de dados (`planets_df`) com base no fato de uma determinada condição ser verdadeira ou não (com ou sem anéis) e conseguiu extrair todos os dados relevantes. Muito legal! A esta altura, o NASA provavelmente já está flertando com o seu CV;-). 
...
success_msg("Maravilhoso! Este exercício conclui o capítulo sobre data frames. Lembre-se de que data frames são extremamente importantes no R, você precisará deles o tempo todo. Outra estrutura de dados muito usada é a lista. Este será o assunto do próximo capítulo!")
```
