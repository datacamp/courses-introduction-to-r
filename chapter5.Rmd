...
*** =instructions
Have a look at the code on the right: it selects the `name` column of all planets that have rings. Adapt the code so that instead of only the `name` column, _all_ columns for planets that have rings are selected.
...
test_output_contains('planets_df[rings_vector, ]', incorrect_msg = "Have you correctly adapated the code to select _all_ columns for the planets that have rings? Remember: to select all columns, you simply have to leave the columns part inside the `[ ]` empty!")
...
*** =instructions
- Call `order()` on the `diameter` column of `planets_df`. Store the result as `positions`.
...
test_object("positions",
            incorrect_msg = "Have you correctly created `positions`? Use `planets_df$diameter`, which selects the diameter column, in combination with `order()`.")
test_output_contains("planets_df[positions,]",
                     incorrect_msg = "Have you correctly sorted `planets_df` and printed out the result? Use `[positions, ]`.")
