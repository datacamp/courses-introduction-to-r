...
- 'How old are your?' (= numeric data type)
...
Working with large datasets is not uncommon in data analysis. When working with (extremely) large datasets and data frames, the first important task of you, the data analyst, is to have a clear understanding of its structure and main elements. Therefore, it is often usefull to show only a small part of the entire dataset. This instead of printing the entire dataset every time you need it. 
...
*** =sct
```{r eval=FALSE}
DM.result <- DM.codeTest("head(mtcars)")
```
...
*** =sct
```{r eval=FALSE}
DM.result <- DM.codeTest("str(mtcars)")
```
...
*** =sct
```{r eval=FALSE}
DM.result <- DM.codeTest("str(planets.df)")
```
