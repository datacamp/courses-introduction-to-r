...
After doing some high-quality research on [NASA](http://nssdc.gsfc.nasa.gov/planetary/factsheet/planet_table_ratio.html), you feel confident enough to create the necessary vectors: `name`, `type`, `diameter`, `rotation` and `rings`; these vectors have already been coded up on the right. The first element in each of these vectors correspond to the first observation.
...
success_msg("Awesome! Notice that the `character` columns were automaticlly converted to `factor`s when you put them inside a `data.frame`. ou can change this behavoir using the `stringsAsFactors` argument.")
```
...
- `my_df[1, 2]` selects the value at the first row and select column in `my_df`.
...
In data analysis you can sort your data according to a certain variable in the data set. In R, this is done with the help of two functions: [`sort()`](http://www.rdocumentation.org/packages/base/functions/sort) and [`order()`](http://www.rdocumentation.org/packages/base/functions/order). 

[`sort()`](http://www.rdocumentation.org/packages/base/functions/sort) does the obvious thing: it sorts the elements of a vector from smallest to largest.
...
* Use `order()` to sort the planet diameters from smallest to largest.
