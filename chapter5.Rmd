...
Cependant, lorsque vous réalisez une étude de marché, vous êtes souvent confronté à des questions telles que celles-ci :
...
Une trame de données présente les variables d'un ensemble de données sous forme de colonnes et les observations sous forme de lignes. Il s'agit d'un concept familier pour celles et ceux qui utilisent d'autres logiciels statistiques tels que SAS ou SPSS.

`@instructions`
Envoyez la réponse. Les données de la trame de données de l'exemple intégré `mtcars` seront affichées sur la console.

`@hint`
Envoyez la réponse et assistez à la magie !
...
Travailler avec de grands ensembles de données n'est pas rare dans l'analyse de données. Lorsque vous travaillez avec des ensembles de données et des trames de données (extrêmement) volumineux, votre première tâche en tant qu'analyste de données est d'acquérir une compréhension claire de leur structure et de leurs principaux éléments. Il est donc souvent utile de ne montrer qu'une petite partie de l'ensemble des données. 

Comment faire cela dans R ? La fonction `head()` vous permet d'afficher les premières observations d'une trame de données. De même, la fonction `tail()` imprime les dernières observations de votre ensemble de données.
...
ex() %>% check_output_expr("head(mtcars)", missing_msg = "Affichez simplement le résultat de l'appel à `head()`, il n'est pas nécessaire de l'assigner à une nouvelle variable.")

success_msg("Magnifique ! Alors, que contient ce jeu de données ? Par exemple, `hp` représente la puissance de la voiture ; la Datsun a la plus faible puissance des 6 voitures affichées. Pour un aperçu complet de la signification des variables, tapez `?mtcars` dans la console et lisez la page d'aide. Continuez à l'exercice suivant !")
```
...
`@sct`
```{r}
msg = "Ne modifiez rien dans la définition des vecteurs. Ajoutez seulement un appel à `data.frame()` pour créer `planets_df`."
...
success_msg("Super ! Maintenant que vous avez une compréhension claire du jeu de données `planets_df`, il est temps de voir comment vous pouvez en sélectionner des éléments. Apprenez tout à ce sujet dans les exercices suivants !")
```
...
Supposons que vous souhaitiez sélectionner les trois premiers éléments de la colonne `type`. Pour ce faire, vous pouvez notamment

```
planets_df[1:3,2]
```

Un inconvénient possible de cette approche est que vous devez connaître (ou rechercher) le numéro de colonne de `type`, ce qui devient difficile si vous avez beaucoup de variables. Il est souvent plus facile d'utiliser le nom de la variable :
...
ex() %>% check_output_expr("planets_df[1:5, \"diameter\"]", missing_msg = "Avez-vous correctement sélectionné les cinq premières valeurs de la colonne diamètre et les avez-vous affichées ? Vous pouvez utiliser `[1:5, \"diameter\"]` ici.")
...
`@instructions`
- Utilisez le signe `$` pour sélectionner la variable `rings` à partir de `planets_df`. Enregistrez le vecteur qui en résulte à l'adresse `rings_vector`.
...
ex() %>% check_output_expr("rings_vector", missing_msg = "N'oubliez pas d'afficher `rings_vector` après l'avoir créé !")
...
success_msg("Magnifique ! C'est une solution plutôt fastidieuse. Le prochain exercice vous apprendra à le faire de manière plus concise.")
```
...
Qu'avez-vous appris exactement dans les exercices précédents ? Vous avez sélectionné un sous-ensemble d'une trame de données (`planets_df`) en fonction de l'existence ou non d'une certaine condition (anneaux ou pas anneaux), et vous avez réussi à extraire toutes les données pertinentes. Plutôt impressionnant ! Désormais la NASA est probablement déjà en train de lorgner sur votre CV ;-). 
...
ex()  %>% check_correct({
  ex() %>% check_output_expr("subset(planets_df, subset = diameter < 1)", missing_msg = "Avez-vous correctement spécifié la partie `subset = ...` à l'intérieur de `subset()`. La condition dans ce cas est `diameter < 1`. Affichez simplement le résultat.")  
...
success_msg("Magnifique ! Cet exercice conclut le chapitre sur les data frames. Rappelez-vous que les data frames sont extrêmement importants en R, vous en aurez besoin tout le temps. Une autre structure de données très souvent utilisée est la liste. Ce sera le sujet du prochain chapitre !")
```
