...
Editörde üç vektör tanımlanmış. Her biri ilk üç Star Wars filminin gişe rakamlarını temsil ediyor. Her vektörün ilk öğesi ABD gişe gelirini, ikinci öğesi ise ABD dışı gişe gelirini gösterir (kaynak: Wikipedia).
...
Üç Star Wars filminin toplam gişe gelirini hesaplamak için, ABD gelir sütunu ile ABD dışı gelir sütununun toplamını almanız gerekir. 
...
`@instructions`
- ABD ve ABD dışı bölgelerin toplam gelirini hesaplayın ve `total_revenue_vector` adresine atayın. `colSums()` fonksiyonunu kullanabilirsiniz.
...
`@instructions`
- Tüm filmlerin ABD dışı gelirlerini seçin ( `all_wars_matrix`'nin ikinci sütununun tamamı), sonucu `non_us_all` olarak kaydedin.
- `non_us_all` 'da `mean()` adresini kullanarak tüm filmlerin ortalama ABD dışı gelirini hesaplayın. Sonucu yazdırın.
- Bu sefer, `all_wars_matrix`'deki ilk iki filmin ABD dışı gelirlerini seçin. Sonucu `non_us_some` olarak depolayın.
...
`@instructions`
- `all_wars_matrix` 'ı `ticket_prices_matrix` ile bölerek altı filmin ABD ve ABD dışı ziyaretçi sayısının tahmini değerini elde edin. Sonucu `visitors` değişkenine atayın.
...
- ABD'den gelen ziyaretçilerin ortalama sayısını hesaplayın; sonucu yazdırın.

`@hint`
- Fonksiyona eklenen girdilerin ortalamasını hesaplamak için `mean()` fonksiyonunu kullanabilirsiniz.
- ABD'deki ziyaretçi sayısını öğrenmek için, `visitors` adresinden `visitors[ ,1]` kullanarak ilk sütunu seçin.
