...
En R, una matriz es una colección de elementos del mismo tipo de datos (numéricos, de caracteres o lógicos) dispuestos en un número fijo de filas y columnas. Como sólo trabajas con filas y columnas, una matriz se llama bidimensional. 
...
- El argumento `byrow` indica que la matriz se rellena con las filas. Si queremos que la matriz se rellene con las columnas, basta con colocar `byrow = FALSE`. 
...
`@hint`
Lee atentamente la tarea, ¡la respuesta ya está dada!
...
En el editor se definen tres vectores. Cada una representa las cifras de taquilla de las tres primeras películas de Star Wars. El primer elemento de cada vector indica los ingresos de taquilla de US, el segundo elemento se refiere a la taquilla de NoUS (fuente: Wikipedia).
...
Para ayudarte a recordar lo que está almacenado en `star_wars_matrix`, te gustaría añadir los nombres de las películas para las filas. Esto no sólo te ayuda a leer los datos, sino que también es útil para seleccionar determinados elementos de la matriz. 
...
ex() %>% check_object("star_wars_matrix") %>% check_equal(eq_condition = "equal",incorrect_msg = "¿Has establecido correctamente los nombres de las filas y columnas de `star_wars_matrix`? Usa `colnames(star_wars_matrix) <- region` para los nombres de las columnas; haz algo similar para nombrar las filas.")
...
Para calcular los ingresos totales de taquilla de las tres películas de Star Wars, tienes que tomar la suma de la columna de ingresos de US y la columna de ingresos que no son deUS. 
...
`@sct`
```{r}
msg <- "¡No cambies nada sobre las variables preestablecidas `box_office_all` y `star_wars_marix`!"
...
msg <- "¿Has usado correctamente `cbind()` para añadir `worldwide_vector` a `star_wars_matrix`? Debes pasar `star_wars_matrix` y `world_wide_vector` a `cbind()`, en este orden. La matriz resultante, `all_wars_matrix`, debe consistir en tres filas y tres columnas."
ex() %>% check_object("all_wars_matrix") %>% check_equal(incorrect_msg = msg)

success_msg("¡Buen trabajo! Después de añadir una columna a una matriz, el siguiente paso lógico es añadir filas. Aprende cómo en el próximo ejercicio.");
```
...
Tu espacio[de](http://www.statmethods.net/interface/workspace.html) trabajo de R, donde "viven" todas las variables que has definido[(consulta qué es un espacio de trabajo](http://www.statmethods.net/interface/workspace.html)), ya se ha inicializado y contiene dos matrices:
...
ex() %>% check_object("all_wars_matrix") %>% check_equal(incorrect_msg = "¿Usaste `rbind()` correctamente para crear `all_wars_matrix()`? `rbind()` debe tomar dos argumentos: `star_wars_matrix` y `star_wars_matrix2`, en este orden.")
...
`@instructions`
- Calcula los ingresos totales de la región US y de la región noUS y asigna `total_revenue_vector`. Puedes utilizar la función `colSums()`.
...
ex() %>% check_function("colSums") %>% check_arg('x') %>% check_equal(incorrect_msg = "¿Has usado la función `colSums()` en el all_wars_matrix?")
...
`@instructions`
- Selecciona los ingresos noUS de todas las películas (toda la segunda columna de `all_wars_matrix`), almacena el resultado como `non_us_all`.
- Utiliza `mean()` en `non_us_all` para calcular la media de ingresos sinUS de todas las películas. Sólo tienes que imprimir el resultado de .
- Esta vez, selecciona los ingresos noUS de las dos primeras películas en `all_wars_matrix`. Guarda el resultado como `non_us_some`.
...
`@instructions`
- Divide `all_wars_matrix` por `ticket_prices_matrix` para obtener el número estimado de visitantes US y noUS de las seis películas. Asigna el resultado a `visitors`.
- En la matriz `visitors`, selecciona toda la primera columna, que representa el número de visitantes en US. Guarda esta selección como `us_visitors`.
- Calcula el número medio de visitantes de US; imprime el resultado.

`@hint`
- Puedes utilizar la función `mean()` para calcular la media de las entradas de la función.
- Para obtener el número de visitantes en US, selecciona la primera columna de `visitors` utilizando `visitors[ ,1]`.
...
ex() %>% check_object("visitors") %>% check_equal(incorrect_msg = "¿Has creado correctamente la matriz `visitors`? Deberías dividir `all_wars_matrix` por `ticket_prices_matrix` para obtenerla.")
