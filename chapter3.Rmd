...
Nell'editor vengono definiti tre vettori. Ognuno di essi rappresenta i numeri degli incassi dei primi tre film di Star Wars. Il primo numero di ogni riga dice quanto ha incassato al botteghino negli Stati Uniti, mentre il secondo numero è quanto ha fatto fuori dagli Stati Uniti (fonte: Wikipedia).
...
Per calcolare il totale degli incassi al botteghino dei tre film di Star Wars, devi mettere insieme la colonna degli incassi negli Stati Uniti e quella degli incassi fuori dagli Stati Uniti. 
...
`@instructions`
- Calcola il fatturato totale per gli Stati Uniti e il resto del mondo e assegna un `total_revenue_vector`. Puoi utilizzare la funzione `colSums()`.
...
`@instructions`
- Scegli i ricavi non statunitensi per tutti i film (tutta la seconda colonna di `all_wars_matrix`), salva il risultato come `non_us_all`.
- Usa `mean()` su `non_us_all` per calcolare la media dei ricavi non statunitensi per tutti i film. Quindi stampa il risultato.
- Questa volta, scegliamo gli incassi fuori dagli Stati Uniti dei primi due film di " `all_wars_matrix`". Memorizza il risultato come `non_us_some`.
...
`@instructions`
- Dividi `all_wars_matrix` per `ticket_prices_matrix` per vedere quanti spettatori americani e non americani ci sono stati per i sei film. Assegna il risultato a `visitors`.
- Dalla matrice " `visitors` ", prendi tutta la prima colonna, che mostra il numero di visitatori negli Stati Uniti. Salva questa selezione come `us_visitors`.
...
- Per vedere quanti visitatori ci sono negli Stati Uniti, scegli la prima colonna da `visitors` usando `visitors[ ,1]`.
