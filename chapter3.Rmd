...
En R, une matrice est une collection d'éléments du même type de données (numériques, caractères ou logiques) disposés en un nombre fixe de lignes et de colonnes. Comme vous ne travaillez qu'avec des lignes et des colonnes, une matrice est dite bidimensionnelle. 
...
ex() %>% check_function("matrix") %>% {
	check_arg(., 'data') %>% check_equal(incorrect_msg = "Avez-vous correctement créé la matrice ? Jetez un œil à l'énoncé, la réponse est déjà donnée !")
	check_arg(., 'byrow') %>% check_equal(incorrect_msg = "Avez-vous correctement créé la matrice ? Jetez un œil à l'énoncé, la réponse est déjà donnée !")
	check_arg(., 'nrow') %>% check_equal(incorrect_msg = "Avez-vous correctement créé la matrice ? Jetez un œil à l'énoncé, la réponse est déjà donnée !")
	}

ex() %>% check_output_expr("matrix(1:9, byrow=TRUE, nrow=3)",missing_msg = "Il semble y avoir un problème avec la définition de la matrice. Jetez un œil à l'énoncé, la réponse est déjà donnée !")
...
Dans l'éditeur, trois vecteurs sont définis. Chacune représente les chiffres du box-office des trois premiers films Star Wars. Le premier élément de chaque vecteur indique les recettes du box-office US, le deuxième élément se réfère au box-office hors US (source : Wikipedia).
...
ex() %>% check_object("star_wars_matrix") %>% check_equal(incorrect_msg = "Avez-vous assigné le résultat de l'appel à `matrix()` à `star_wars_matrix` ?")

success_msg("La force est effectivement avec vous ! Continuez vers l'exercice suivant.")
```
...
ex() %>% check_output_expr("star_wars_matrix", missing_msg = "N'oubliez pas d'afficher `star_wars_matrix` après avoir nommé les lignes et les colonnes.")
...
Pour calculer les recettes totales des trois films Star Wars, vous devez faire la somme de la colonne des recettes aux US et de la colonne des recettes hors US. 
...
success_msg("Bien joué ! Continuez vers l'exercice suivant.")
```
...
`@instructions`
- Calculez le revenu total pour la région US et la région nonUS et affectez `total_revenue_vector`. Vous pouvez utiliser la fonction `colSums()`.
...
ex() %>% check_function("colSums") %>% check_arg('x') %>% check_equal(incorrect_msg = "Avez-vous utilisé la fonction `colSums()` sur `all_wars_matrix` ?")
...
ex() %>% check_output_expr("total_revenue_vector", missing_msg = "N'oubliez pas d'afficher `total_revenue_vector` !")
...
`@instructions`
- Sélectionnez les recettes autres que celles deUS pour tous les films (l'ensemble de la deuxième colonne de `all_wars_matrix`) et enregistrez le résultat sous `non_us_all`.
- Utilisez `mean()` sur `non_us_all` pour calculer le revenu moyen hors US pour tous les films. Il suffit d'afficher le résultat.
- Cette fois, sélectionnez les recettes autres que celles des US pour les deux premiers films de `all_wars_matrix`. Enregistrez le résultat en tant que `non_us_some`.
...
ex() %>% check_object("non_us_all") %>% check_equal(incorrect_msg = "Avez-vous assigné à `non_us_all` l'intégralité de la deuxième colonne de `all_wars_matrix` ? Vous pouvez utiliser `[, 2]` pour le faire !")
...
ex() %>% check_object("non_us_some") %>% check_equal(incorrect_msg = "Avez-vous assigné à `non_us_some` les revenus hors US pour les deux premiers films ? Vous pouvez utiliser `[1:2,2]` pour le faire !")
...
ex() %>% check_object("visitors") %>% check_equal(incorrect_msg = "Il semble que `visitors` ne soit pas correct. Divisez simplement `all_wars_matrix` par 5 et stockez la matrice résultante sous le nom `visitors`.")

ex() %>% check_output_expr("visitors", missing_msg = "N'oubliez pas d'imprimer également `visitors` pour y jeter un coup d'œil.")
...
`@instructions`
- Divisez `all_wars_matrix` par `ticket_prices_matrix` pour obtenir le nombre estimé de visiteurs aux US et hors US pour les six films. Affectez le résultat à `visitors`.
- Dans la matrice `visitors`, sélectionnez toute la première colonne, qui représente le nombre de visiteurs aux US. Enregistrez cette sélection dans `us_visitors`.
- Calculez le nombre moyen de visiteurs aux US ; affichez le résultat.

`@hint`
- Vous pouvez utiliser la fonction `mean()` pour calculer la moyenne des entrées de la fonction.
- Pour obtenir le nombre de visiteurs aux US, sélectionnez la première colonne de `visitors` à l'aide de `visitors[ ,1]`.
