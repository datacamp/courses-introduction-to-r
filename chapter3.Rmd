...
- Construa uma matriz com 3 linhas, em que cada linha representa um filme. Use a função `matrix()` para fazer isso. O primeiro argumento é o vetor `box_office`, que contém todos os números de bilheteria. Em seguida, você terá que especificar `nrow = 3` e `byrow = TRUE`. Chame a matriz resultante de .
...
`@sct`
```{r}
msg <- "Não altere nada sobre as variáveis de bilheteria `new_hope`, `empire_strikes` e `return_jedi`!"
...
ex() %>% check_object("star_wars_matrix") %>% check_equal(incorrect_msg = "Você atribuiu o resultado da chamada `matrix()` a `star_wars_matrix`?")
...
msg <- "Não altere o conteúdo de `star_wars_matrix`; apenas os nomes das linhas e colunas!" 

ex()  %>% check_object("star_wars_matrix") %>% check_equal(incorrect_msg = msg)

msg <- "Não altere nada sobre os vetores `region` e `titles` que foram definidos para você."
...
success_msg("Ótimo! Você está no caminho para se tornar um jedi em R! Continue para o próximo exercício.")
```
...
Assim como toda ação tem uma reação, todo ``rbind()``cbind()` tem um  (admitimos que somos muito ruins com metáforas). 
...
`@instructions`
Use `rbind()` para colar `star_wars_matrix` e `star_wars_matrix2`, nesta ordem. Atribua a matriz resultante a `all_wars_matrix`.
...
`@sct`
```{r}
msg = "Não substitua as variáveis que foram definidas para você no workspace (`star_wars_matrix` e `star_wars_matrix2`)."
...
success_msg("Maravilhoso! Continue com o próximo exercício e veja como você pode combinar os resultados da função `rbind()` com a função `colSums()`!")
```
...
Assim como ``rbind()``cbind()` tem ``rowSums()``colSums()`,  tem . Seu espaço de trabalho do R já contém `all_wars_matrix`, que você construiu no exercício anterior; digite `all_wars_matrix` para dar outra olhada. Agora vamos calcular a receita total de bilheteria de toda a saga.

`@instructions`
- Calcule a receita total nos EUA e fora dos EUA e atribua o resultado a US. Você pode usar a função .
- Imprima `total_revenue_vector` para dar uma olhada nos resultados.

`@hint`
Você deve usar a função ``star_wars_matrix``colSums()` com  como argumento para chegar à bilheteria total por região.
...
success_msg("Bellissimo! Vá para o próximo exercício para aprender a fazer subsetting em matrizes.")
```
...
Assim como nos vetores, podemos usar colchetes `[ ]` para selecionar um ou vários elementos de uma matriz. Enquanto os vetores têm uma dimensão, as matrizes têm duas dimensões. Portanto, devemos usar vírgula para separar as linhas que desejamos selecionar nas colunas. Por exemplo: 
...
`@instructions`
- Selecione a receita de todos os filmes fora dos EUA (a segunda coluna inteira de US) e armazene o resultado como `all_wars_matrix`.
- Use `mean()` com `non_us_all` para calcular a receita média de todos os filmes fora dos EUA. Imprima o resultado de novo.
- Dessa vez, selecione a receita fora dos EUA referente aos dois primeiros filmes em US. Armazene o resultado como `non_us_some`.
...
`@sct`
```{r}
msg = "Não altere o conteúdo de `all_wars_matrix`; esta matriz já foi criada para você no workspace."

ex() %>% check_object("all_wars_matrix", undefined_msg = msg) %>% check_equal(incorrect_msg = msg)

ex() %>% check_object("non_us_all") %>% check_equal(incorrect_msg = "Você atribuiu a `non_us_all` toda a segunda coluna de `all_wars_matrix`? Você pode usar `[, 2]` para fazer isso!")
...
ex() %>% check_object("non_us_some") %>% check_equal(incorrect_msg = "Você atribuiu a `non_us_some` a receita fora dos EUA para os dois primeiros filmes? Você pode usar `[1:2,2]` para fazer isso!")
...
`@instructions`
- Divida `all_wars_matrix` por 5, obtendo o número de visitantes em milhões. Atribua a matriz resultante a `visitors`.
...
ex() %>% check_object("visitors") %>% check_equal(incorrect_msg = "Parece que `visitors` não está correto. Simplesmente divida `all_wars_matrix` por 5 e armazene a matriz resultante como `visitors`.")

ex() %>% check_output_expr("visitors", missing_msg = "Não se esqueça de também imprimir `visitors` para que você possa dar uma olhada.")


success_msg("Ótimo! O que esses resultados dizem a você? Um impressionante número de 92 milhões de pessoas foram ver A New Hope nos cinemas dos EUA! Continue para o próximo exercício.")
```
...
`@instructions`
- Divida `all_wars_matrix` por `ticket_prices_matrix` para obter o número estimado de espectadores nos EUA e fora dos EUA para os seis filmes. Atribua o resultado a `visitors` (espectadores).
...
- Para obter o número de espectadores nos EUA, selecione a primeira coluna de US usando `visitors`.
...
ex() %>% check_output_expr("mean(us_visitors)", missing_msg = "Depois de criar `us_visitors`, você pode usar `mean()` para calcular o número médio de visitantes nos EUA. Certifique-se de imprimir o resultado.")


success_msg("É um fato: a força do R está com você! Este exercício conclui o capítulo sobre matrizes. Próxima parada na sua jornada pela linguagem R: fatores.")
```
