...
En el editor se definen tres vectores. Cada una representa las cifras de taquilla de las tres primeras películas de Star Wars. El primer elemento de cada vector indica los ingresos de taquilla en EE. UU., mientras que el segundo elemento se refiere a la taquilla fuera de EE. UU. (fuente: Wikipedia).
...
Para calcular los ingresos totales de taquilla de las tres películas de Star Wars, hay que sumar la columna de ingresos en EE. UU. y la columna de ingresos fuera de EE. UU. 
...
`@instructions`
- Calcula los ingresos totales para EE. UU. y el resto del mundo y asigna un `total_revenue_vector`. Puedes utilizar la función `colSums()`.
...
`@instructions`
- Selecciona los ingresos fuera de EE. UU. de todas las películas (toda la segunda columna de `all_wars_matrix`) y guarda el resultado como `non_us_all`.
- Utiliza `mean()` en `non_us_all` para calcular los ingresos medios fuera de EE. UU. de todas las películas. Solo tienes que imprimir el resultado.
- Esta vez, selecciona los ingresos fuera de EE. UU. de las dos primeras películas en `all_wars_matrix`. Guarda el resultado como `non_us_some`.
...
`@instructions`
- Divide « `all_wars_matrix` » (Público total de EE. UU. en 2012) entre « `ticket_prices_matrix` » (Público total de EE. UU. en 2011) para obtener el número estimado de espectadores estadounidenses y no estadounidenses de las seis películas. Asigna el resultado a `visitors`.
...
- Calcula el número medio de visitantes estadounidenses e imprime el resultado.
