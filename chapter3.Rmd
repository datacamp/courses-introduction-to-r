...
test_object("all_wars_matrix", incorrect_msg = "Did you use the `rbind()` correctly to create `all_wars_matrix()`? `rbind()` should take two arugments: `star_wars_matrix` and `star_wars_matrix2`, in this order.")
...
*** =pre_exercise_code
```{r}
download.file("http://s3.amazonaws.com/assets.datacamp.com/course/intro_to_r/all_wars_matrix.rds", "all_wars_matrix.rds")
all_wars_matrix <- readRDS("all_wars_matrix.rds")
```
...
test_output_contains("mean(non_us)",
...
test_output_contains("mean(us_visitors)", incorrect_msg = "Once you have created `us_visitors`, you can use `mean()` to calcualte the average number of visitors in the US. Make sure to print out the result.")
