...
`@instructions`
Utiliza `rbind()` para pegar `star_wars_matrix` y `star_wars_matrix2`, en este orden. Asigna la matriz resultante a .
...
`@instructions`
- Selecciona los ingresos que no sean US de todas las películas (toda la segunda columna de `all_wars_matrix`), almacena el resultado como `non_us_all`.
- Utiliza `mean()` en `non_us_all` para calcular la media de ingresos sinUS de todas las películas. Solo tienes que imprimir el resultado.
- Esta vez, selecciona los ingresos que no sean US de las dos primeras películas en `all_wars_matrix`. Guarda el resultado como `non_us_some`.
...
`@instructions`
- Divide `all_wars_matrix` por 5, lo que te dará el número de visitantes en millones. Asigna la matriz resultante a .
...
`@instructions`
- Divide `all_wars_matrix` por `ticket_prices_matrix` para obtener el número estimado de visitantes US y no US de las seis películas. Asigna el resultado a `visitors`.
