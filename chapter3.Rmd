...
Dans l'éditeur, trois vecteurs sont définis. Chacune représente les chiffres du box-office des trois premiers films Star Wars. Le premier élément de chaque vecteur indique les recettes au box-office américain, le deuxième élément fait référence au box-office hors États-Unis (source : Wikipedia).
...
Pour calculer le total des recettes au box-office des trois films Star Wars, il est nécessaire d'additionner la colonne des recettes aux États-Unis et la colonne des recettes hors États-Unis. 
...
`@instructions`
- Veuillez calculer le chiffre d'affaires total pour les États-Unis et les autres régions, puis attribuer l'`total_revenue_vector`. Vous pouvez utiliser la fonction `colSums()`.
...
`@instructions`
- Veuillez sélectionner les recettes hors États-Unis pour tous les films (l'intégralité de la deuxième colonne de `all_wars_matrix`), puis enregistrez le résultat sous le nom `non_us_all`.
- Veuillez utiliser `mean()` sur `non_us_all` pour calculer le revenu moyen hors États-Unis pour tous les films. Il suffit d'afficher le résultat.
- Cette fois-ci, veuillez sélectionner les recettes hors États-Unis pour les deux premiers films dans `all_wars_matrix`. Enregistrez le résultat en tant que `non_us_some`.
...
- Dans la matrice `visitors`, veuillez sélectionner l'intégralité de la première colonne, qui représente le nombre de visiteurs aux États-Unis. Enregistrez cette sélection dans `us_visitors`.
- Veuillez calculer le nombre moyen de visiteurs américains et imprimer le résultat.

`@hint`
- Vous pouvez utiliser la fonction `mean()` pour calculer la moyenne des entrées de la fonction.
- Pour obtenir le nombre de visiteurs aux États-Unis, veuillez sélectionner la première colonne de `visitors` en utilisant `visitors[ ,1]`.
