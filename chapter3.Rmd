...
`star_wars_matrix` has been predefined for you without any row or column names.
...
`star_wars_matrix` has been predefined for you.

* Use `rowSums()` to calculate the worldwide box office figures for the three movies and put these in the vector named `worldwide_vector`.
...
*** =sct
```{r}
msg <- "Do not change anything about the preset variable `star_wars_marix`!"
...
success_msg("Great, kid. Don't get cocky!.")
```
